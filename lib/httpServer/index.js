  ...
  constructor (node, blockchain, operator, miner) {
    ...
    this.app.get('/operator/wallets/:walletId/addresses', (req, res) => {
      ...
      try {
        ...
        if (addresses === null || addresses === undefined || addresses.length === 0) throw new HTTPError(400, `Wallet '${walletId}' does not exist, cannot get addresses`)
    ...
    this.app.post('/operator/wallets/:walletId/addresses', (req, res) => {
      ...
      if (password === null || password === undefined || password.length < 1 || walletId === null || walletId === undefined || walletId.length < 64) throw new HTTPError(400, 'Wallet\'s ID or Password is either missing or an incorrect format')
      let passwordHash = CryptoUtil.hash(password)
