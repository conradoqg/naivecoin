  ...
  constructor (node, blockchain, operator, miner) {
    ...
    this.app.get('/operator/wallets/:walletId/addresses', (req, res) => {
      ...
      try {
        ...
        if (addresses === null || addresses === undefined || !addresses[0]) throw new HTTPError(400, `Wallet '${walletId}' does not exist, cannot get addresses`)
    ...
    this.app.post('/operator/wallets/:walletId/addresses', (req, res) => {
      ...
      if (password === null || password === undefined || password.length < 64) throw new HTTPError(403, 'Wallet\'s Password is either missing or an incorrect format')
      if (walletId === null || walletId === undefined || walletId.length < 64) throw new HTTPError(400, 'Wallet\'s ID is either missing or an incorrect format')
      let passwordHash = CryptoUtil.hash(password)
