...
class Wallet {
    ...
    generateAddress() {
        // TODO: Define a way to return an address
        if (this.secret == null) {
            this.generateSecret();
        }

        let lastKeyPair = R.last(this.keyPairs);
        let seed = R.defaultTo(this.secret, R.propOr(null, 'publicKey', lastKeyPair));
    ...
    getAddresses() {
        return R.map((keyPair) => { return keyPair.publicKey; }, this.keyPairs);
    }
