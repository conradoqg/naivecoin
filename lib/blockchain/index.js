...
class Blockchain {
    ...
    isValidChain(blockchainToValidate) {
        ...
        for (let i = 1; i < blockchainToValidate.length; i++) {
            if (!this.isBlockValid(Block.fromJson(blockchainToValidate[i]), blockchainToValidate[i - 1])) {
                return false;
            }
        }
        return true;
    ...
    addBlock(newBlock, emit = true) {
        if (this.isBlockValid(newBlock, this.getLastBlock())) {
    ...
    isBlockValid(newBlock, previousBlock) {
        // TODO: This validation should throw exceptions
    ...
    checkTransaction(transaction) {
        ...
        if (!isNotInBlockchain) throw new TransactionAssertionError(`Transaction '${transaction.id}' is already in the blockchain`, transaction);
        ...
        if (!isInputTransactionsUnspent) throw new TransactionAssertionError(`Not all inputs are unspent for transaction '${transaction.id}'`, transaction.data.inputs);
