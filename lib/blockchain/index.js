...
class Blockchain {
    ...
    getDifficulty(index) {
        // INFO: The difficulty is the formula that naivecoin choose to check the prove a work, this number is later converted to base 16 to represent the minimal initial hash expected value
    ...
    checkBlock(newBlock, previousBlock) {
        ...
        } else if (newBlock.getDifficulty() >= this.getDifficulty(newBlock.index)) { // If the difficulty level of the prove-of-work challenge is correct
        ...
        if (transactionsByType.fee && transactionsByType.fee > 1) {
            ...
            throw new TransactionAssertionError(`Invalid fee transaction count: expected '1' got '${transactionsByType.fee}'`);
        }
        ...
        if (transactionsByType.reward && transactionsByType.reward > 1) {
            ...
            throw new TransactionAssertionError(`Invalid reward transaction count: expected '1' got '${transactionsByType.reward}'`);
        }
