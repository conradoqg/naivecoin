...
class Blockchain {
    ...
    checkBlock(newBlock, previousBlock) {
        ...
        if (previousBlock.index + 1 !== newBlock.index) {
            ...
            console.error(`Invalid prove of work difficulty: expected '${newBlock.getDifficulty()}' to be smaller than '${this.getDifficulty(newBlock.index)}'`);
        ...
        R.map(this.checkTransaction.bind(this), newBlock.transactions);

        let sumOfInputsAmount = R.sum(R.flatten(R.map(R.compose(R.map(R.prop('amount')), R.prop('inputs'), R.prop('data')), newBlock.transactions))) + 5000000000;
        ...
        if (!isInputsAmountGreaterOrEqualThanOutputsAmount) {
            ...
            throw new TransactionAssertionError(`Invalid block balance, inputs sum '${sumOfInputsAmount}', outputs sum '${sumOfOutputsAmount}'`, { sumOfInputsAmount, sumOfOutputsAmount });
        }

        let transactionsByType = R.countBy(R.prop('type'), newBlock.transactions);
        if (transactionsByType.fee && transactionsByType.fee > 1) {
            console.error(`Invalid fee transaction count, expected 1, found ${transactionsByType.fee}`);
            throw new TransactionAssertionError(`Invalid fee transaction count, expected 1, found ${transactionsByType.fee}`);
        }

        if (transactionsByType.fee && transactionsByType.reward > 1) {
            console.error(`Invalid reward transaction count, expected 1, found ${transactionsByType.fee}`);
            throw new TransactionAssertionError(`Invalid reward transaction count, expected 1, found ${transactionsByType.fee}`);
        }
