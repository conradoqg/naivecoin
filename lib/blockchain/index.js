...
class Blockchain {
    ...
    checkBlock(newBlock, previousBlock) {
        ...
        if (transactionsByType.reward && transactionsByType.reward > 1) {
            console.error(`Invalid reward transaction count, expected 1, found ${transactionsByType.reward}`);
            throw new TransactionAssertionError(`Invalid reward transaction count, expected 1, found ${transactionsByType.reward}`);
        }
