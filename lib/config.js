    ...
    pow: {
        getDifficulty: function getDifficulty(blocks, index) {
            ...
            if (index > 2 * AVERAGE_X_BLOCKS) {
                ...
                //console.log("start block timestamp - Difficulty - BlockIndex " + blocks[avg_start_index].timestamp + "-" + parseInt(blocks[avg_start_index].hash.substring(0, 14), 16) + " - " + avg_start_index);
                //console.log("stop block timestamp - Difficulty - BlockIndex " + blocks[avg_stop_index].timestamp + "-" + parseInt(blocks[avg_stop_index].hash.substring(0, 14), 16) + " - " + avg_stop_index);
                ...
                let odiff_sum = 0;
                for(let i = avg_start_index; i <= avg_stop_index; i++) odiff_sum += parseInt(blocks[i].hash.substring(0,14), 16);
                let old_difficulty = getDifficulty(blocks, avg_start_index); //Calculate average of prior difficulties
                //console.log("old_difficulty: " + old_difficulty);
                let new_difficulty = Math.min(Math.max(Math.floor(old_difficulty * (scale_factor)), 0), BASE_DIFFICULTY);
