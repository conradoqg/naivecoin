...
class Node {
    constructor(host, port, peers, blockchain, proxyUrl) {
        this.host = host;
        this.port = port;
        this.peers = [];
    ...
    connectToPeers(newPeers) {
        ...
        let me = this.proxyUrl === null ? `http://${this.host}:${this.port}` : this.proxyUrl;
    ...
    processHeartbeat(peer, isAlive) {
        if (!isAlive) {
            this.peers = this.peers.filter(x => x.url !== peers.url); // Remove peer from peer list if it's not alive.
        }
    }
}
