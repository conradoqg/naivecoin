...
class Node {
    constructor(name, host, port, peers, network, blockchain, broadcastAddress) {
        this.name = name;
        this.host = host;
        this.port = port;
        this.peers = [];
        ...
        this.broadcastAddress = broadcastAddress || `http://${host}:${port}`;
        this.hookBlockchain();
    ...
    connectToPeers(newPeers) {
        ...
        let me = this.broadcastAddress;
        newPeers.forEach((peer) => {
            // If it already has that peer, ignore it.
            if (!this.peers.find((element) => { return element.url == peer.url; }) && peer.url != me) {
                // Get node information
                this.getPeer(peer)
                    .then(res => {
                        const peerInformation = res.body;
                        // Check if it belongs to the same network
                        if (peerInformation.network == this.network) {
                            this.sendPeer(peer, {
                                name: this.name,
    ...
    getPeer(peer) {
        const URL = `${peer.url}/node/peer`;
        console.info(`Getting peer ${URL} information`);
        return superagent
            .get(URL)
            .catch((err) => {
                console.warn(`Unable to get peer ${URL} details: ${err.message}`);
            });
    }
    ...
    sendPeer(peer, peerToSend) {
        ...
        return superagent
            ...
            .send(peerToSend)
            .catch((err) => {
                console.warn(`Unable to send ${peerToSend.url} to peer ${URL}: ${err.message}`);
            });
    }
