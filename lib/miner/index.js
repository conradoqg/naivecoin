...
class Miner {
  ...
  static generateNextBlock (rewardAddress, feeAddress, blockchain) {
    ...
    // Add fee transaction if regular transactions are present in the block
    ...
    if (rewardAddress != null) {
      ...
      // Check if block is the Premine block and if so, set the reward to PREMINE_REWARD
      if (blocks.length === 1) rewardTransaction.data.outputs[0].amount = Config.PREMINE_REWARD
  ...
  static proveWorkFor (jsonBlock, difficulty) {
    ...
    do {
      ...
      this.secondlastHashtime = (this.lastHashtime || new Date().getTime());
      this.lastHashtime = new Date().getTime();
  ...
  getCurrentHashrate() {
    let lastHashtime = (Miner.lastHashtime || new Date().getTime());
    let secondlastHashtime = (Miner.secondlastHashtime || new Date().getTime());
    return 1/((secondlastHashtime - lastHashtime)/1000);
  }
}
