...
class Miner {
    ...
    mine(rewardAddress, feeAddress) {
        ...
        return { promise: promise, thread: thread };
    }
    ...
    static generateNextBlock(rewardAddress, feeAddress, blockchain) {
        ...
        const timestamp = new Date().getUTCTime() / 1000;
    ...
    static proveWorkFor(jsonBlock, difficulty) {
        ...
        function updateTimestamp(block) {
            block.deleteHashCache();
            block.timestamp = new Date().getUTCTime() / 1000;
        }

        // Loop incrementing the nonce to find the hash at desired difficulty
        do {
            if (block.nonce % 250000 === 0) {
                console.info(`Mining in progress, Current nonce: ${block.nonce}...`);
            }

            if (block.nonce % 1000 === 0) { // Update timestamp every 1000th nonce
                updateTimestamp(block);
            }

            block.nonce++;
            block.hash = block.toHash(true); // Use cached hashing method
