...
class PeerToPeer {
    ...
    connectToPeers(newPeers) {
        newPeers.forEach((peer) => {
            if (!this.peers.find((element) => { return element.url == peer.url; })) {
                const URL = `${peer.url}/peers/new`;
                superagent
                    .post(URL)
                    .send({ url: `http://localhost:${this.port}` })
                    .then(() => {
    ...
    checkReceivedBlocks(blocks) {
        ...
        const latestBlockHeld = this.blockchain.last();
