...
describe('Integration Test', () => {
  ...
  const createConcord = (name, host, port, peers, removeData = true) => {
  ...
  step('start server 1', () => {
    return createConcord(name1, 'localhost', 3001, [])
      .then((httpServer) => {
  ...
  step('start server 2', () => {
    return createConcord(name2, 'localhost', 3002, [{ url: 'http://localhost:3001' }])
      .then((httpServer) => {
  ...
  describe('Complementary tests', () => {
    ...
    step('restart server 1', () => {
      return Promise.resolve()
        ...
        .then(() => {
          return createConcord(name1, 'localhost', 3001, [], false)
            .then((httpServer) => {
              context.httpServer1 = httpServer
            })
        })
        ...
        .then(() => {
          return supertest(context.httpServer1.app)
            ...
            .send({
              fromAddress: context.address1,
              toAddress: context.address2,
              amount: Config.PREMINE_REWARD * 2,
              changeAddress: context.address1
            })
            .expect(400)
        })
